...
const Header = () => {
  ...
  return (
    <div className="w-full relative z-[100]">
      ...
      {/* Top Utility Bar — scrolls away naturally, only the nav below is sticky */}
      <div className="w-full bg-gradient-to-r from-[#854D0E] via-[#9C5A10] to-[#A16207] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-10 sm:h-11 flex items-center justify-between gap-3 text-[12px] sm:text-[13px] font-bold tracking-wide">

          {/* Left: Submit an Article */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <span className="hidden xs:inline sm:inline whitespace-nowrap">Submit an Article</span>
            <Link
              href="/submit"
              className="flex-shrink-0 bg-white text-[#854D0E] text-[11px] sm:text-[12px] font-black px-3 sm:px-4 py-1 sm:py-1.5 rounded-full shadow-sm hover:bg-[#F0FDF4] hover:text-[#16A34A] hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 whitespace-nowrap"
            >
              Submit Now
            </Link>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 justify-end">
            <span className="hidden md:inline whitespace-nowrap text-white/90">
              Facing an issue? Contact
            </span>
            <a
              href="mailto:info@mparesearch.com"
              className="flex-shrink-0 flex items-center gap-1.5 bg-white/15 hover:bg-white text-white hover:text-[#854D0E] text-[11px] sm:text-[12px] font-black px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border border-white/30 hover:border-white transition-all duration-300 whitespace-nowrap"
            >
              <Mail size={13} className="opacity-90" />
              <span className="hidden sm:inline">info@mparesearch.com</span>
      ...
      {/* Main Navigation — sticky, glassy on scroll */}
      <header className="w-full bg-white/90 backdrop-blur-md border-b border-gray-100 sticky top-0 shadow-[0_1px_0_rgba(0,0,0,0.04)] transition-shadow duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center h-16 md:h-[76px] transition-all duration-300">

          {/* Logo Section - Responsive Widths */}
          <Link href="/" className="relative w-32 h-8 sm:w-40 sm:h-10 md:w-56 md:h-12 transition-transform duration-300 active:scale-95 hover:scale-[1.02]">
            <Image src="/newLogo.png" alt="MPA Research Logo" fill className="object-contain" priority />
          </Link>
            ...
            <div className="flex items-center gap-4 xl:gap-6 border-r border-gray-100 pr-6">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="group relative text-[#854D0E] text-[14px] xl:text-[15px] font-bold whitespace-nowrap py-1"
                >
                  <span className="transition-colors duration-300 group-hover:text-[#16A34A]">
                    {link.name}
                  </span>
                  <span className="absolute left-1/2 -bottom-0.5 h-[2px] w-0 bg-[#22C55E] rounded-full transition-all duration-300 ease-out group-hover:w-full group-hover:left-0" />
                </Link>
              ))}
            </div>

            <Link
              href="/menuscript-search"
              className="p-2 hover:bg-gray-50 rounded-full transition-all duration-300 hover:scale-110 active:scale-95"
            >
              <Search className="text-[#854D0E]" size={20} />
            </Link>
            ...
            <div className="flex items-center gap-4">
              {isLoggedIn ? (
                <div className="relative" ref={dropdownRef}>
                  <button
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    className="flex items-center gap-2.5 p-1.5 pr-4 rounded-full border border-gray-200 bg-gray-50 hover:bg-white hover:shadow-md hover:border-gray-300 transition-all duration-300"
                  >
                    <div className="bg-[#22C55E] text-white p-1.5 rounded-full shadow-sm shadow-green-200">
                      <User size={16} />
                    </div>
                    <span className="text-sm font-bold text-gray-700 max-w-[120px] truncate">
                      {isLoading ? "..." : userData?.user?.name || userData?.name || "User"}
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 mt-3 w-52 bg-white border border-gray-100 rounded-2xl shadow-2xl py-2 origin-top-right animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200">
                      <Link href="/dashboard/my-profile" onClick={() => setIsDropdownOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors duration-200">
                        <LayoutDashboard size={18} className="text-[#22C55E]" /> Dashboard
                      </Link>
                      <div className="h-px bg-gray-100 mx-2" />
                      <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-red-600 hover:bg-red-50 transition-colors duration-200">
                        <LogOut size={18} /> Logout
                      </button>
                    </div>
                  )}
                ...
                <Link
                  href="/login"
                  className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-[#854D0E] text-white hover:bg-[#A16207] hover:shadow-lg hover:shadow-yellow-100 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 text-sm font-bold"
                >
                  <User size={18} /> Login
                </Link>
              )}
          ...
          {/* Mobile Buttons */}
          <div className="lg:hidden flex items-center gap-2">
            <Link href="/menuscript-search" className="p-2 text-[#854D0E] hover:bg-gray-100 rounded-full transition-all duration-300 active:scale-90">
              <Search size={22} />
            </Link>
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="p-2 text-[#854D0E] hover:bg-gray-100 rounded-xl transition-all duration-300 active:scale-90"
            >
              <Menu size={28} />
            </button>
          </div>
          ...
          {/* Mobile Sidebar Overlay */}
          <div
            className={`fixed inset-0 bg-gray-900/60 backdrop-blur-md z-[110] transition-opacity duration-500 lg:hidden ${isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
            onClick={() => setIsMobileMenuOpen(false)}
          />

          {/* Mobile Sidebar Menu */}
          <div className={`fixed top-0 right-0 h-[100dvh] w-[85%] max-w-[320px] bg-white z-[120] shadow-[-10px_0_30px_rgba(0,0,0,0.12)] transform transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] lg:hidden flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <div className="p-5 flex justify-between items-center border-b border-gray-50">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-50 text-gray-500 rounded-full hover:bg-gray-100 transition-all duration-300 active:scale-90">
                <X size={20} />
              </button>
            </div>
                ...
                {isLoggedIn ? (
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                    ...
                    <Link
                      href="/login"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-[#854D0E] text-white text-sm font-black shadow-lg shadow-yellow-100 active:scale-95 transition-all duration-300"
                    >
                      <User size={18} /> Login to Account
                    </Link>
                  </div>
                ...
                <div className="flex flex-col space-y-1">
                  {navLinks.map((link) => (
                    <Link
                      ...
                      href={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="px-4 py-3.5 text-[15px] font-bold text-gray-700 hover:text-[#22C55E] hover:bg-gray-50 rounded-xl transition-all duration-300 flex items-center justify-between group"
                    >
                      {link.name}
                      <div className="w-1.5 h-1.5 rounded-full bg-gray-200 group-hover:bg-[#22C55E] group-hover:scale-125 transition-all duration-300" />
                    </Link>
                  ))}
                </div>
              </div>
              ...
              {isLoggedIn && (
                <div className="p-6 bg-gray-50/50 border-t border-gray-50 flex flex-col gap-3">
                  <Link
                    href="/dashboard/my-profile"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-[#22C55E] text-white font-black shadow-lg shadow-green-100 active:scale-95 transition-all duration-300"
                  >
                    <LayoutDashboard size={20} />
                    <span>Dashboard</span>
                  </Link>
                  <button
                    onClick={handleLogout}
                    className="flex items-center justify-center gap-3 w-full py-4 rounded-2xl bg-white text-red-600 font-black border border-red-100 shadow-sm active:scale-95 transition-all duration-300"
                  >
                    <LogOut size={20} />
                    <span>Sign Out</span>
                  </button>
